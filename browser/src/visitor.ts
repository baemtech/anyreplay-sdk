...
/**
 * Removes everything the recorder has stored in this browser.
 *
 * Called when consent is refused. A refusal has to undo an earlier acceptance,
 * not merely stop the current page: the visitor id written last week is
 * exactly what the visitor is now saying no to, and leaving it in place would
 * make "we store nothing without consent" untrue for as long as it sat there.
 * The cool-down goes too — it is a fact about a session that no longer exists.
 * Removing a key that is not there creates nothing, so this is safe to call
 * before anything was ever written.
 */
export function forgetVisitor(local: Storage, session: Storage): void {
  local.remove(VISITOR_KEY);
  for (const key of [SESSION_KEY, SESSION_TS_KEY, SEQ_KEY, PAGE_KEY, COOLDOWN_KEY]) session.remove(key);
}
