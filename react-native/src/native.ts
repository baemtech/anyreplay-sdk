import { createRecorder, type Host, type RecorderHandle } from './recorder.js';
...
function buildHost(options: AnyReplayNativeOptions): Host {
  ...
  const maskTestID = options.maskTestID ?? 'ar-mask';
  const maskImages = options.maskImages === true;
    ...
    snapshot: () => {
      ...
      try {
        return captureTree(root, ids, {
          measure, screen: RN.Dimensions.get('window'), maskAllInputs, maskTestID, maskImages,
...
export async function init(
  ...
  try {
    recorder = await createRecorder(options, host ?? buildHost(options));
...
export function identify(traits: { userId?: string; email?: string }): void { recorder?.identify(traits); }
