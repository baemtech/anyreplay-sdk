...
/**
 * Whether this element's text must never leave the device.
 *
 * Default-deny, like the web recorder: anything a person types is masked unless
 * the customer says otherwise, because recording one password field once is a
 * breach and the cost of the safe default is a configuration line.
 */
export function isMaskedByDefault(tag: MobileTag, props: Record<string, unknown>): boolean {
  if (tag === 'TextInput') return true;
  if (props.secureTextEntry === true) return true;
  const testID = typeof props.testID === 'string' ? props.testID : '';
  return testID.includes('ar-mask');
}
