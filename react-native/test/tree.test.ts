...
import { diff, isEmpty, isMaskedByDefault, serialise, tagFor, type CapturedElement } from '../src/tree.js';
...
describe('masking', () => {
  /**
   * Default-deny, the same rule the web recorder follows. Recording one
   * password field once is a breach; the cost of the safe default is a line of
   * configuration for the rare field that genuinely needs to be visible.
   */
  it('masks anything a person types, without being asked', () => {
    expect(isMaskedByDefault('TextInput', {})).toBe(true);
  });

  it('masks a secure field whatever it calls itself', () => {
    expect(isMaskedByDefault('View', { secureTextEntry: true })).toBe(true);
  });

  it('masks what the customer marked', () => {
    expect(isMaskedByDefault('Text', { testID: 'card-number ar-mask' })).toBe(true);
  });

  it('leaves ordinary text alone', () => {
    expect(isMaskedByDefault('Text', { testID: 'greeting' })).toBe(false);
  });
});
